...
function drawBillSlip(
  ...
  doc.text(school.schoolName || "School Name", x + width / 2, innerY + 3, {
    align: "center",
  });
  doc.setFontSize(7);
  ...
  doc.text(
    `Class: ${bill.class.name}${
      bill.class.section ? `-${bill.class.section}` : ""
    }`,
    innerX,
    innerY + 22
  );
  doc.text(
    `Roll: ${bill.student.rollNumber || "N/A"}`,
    x + width - margin,
    innerY + 22,
    { align: "right" }
  );
  doc.text(
    `Month: ${bill.invoice.month}, ${bill.invoice.year}`,
    x + width - margin,
    innerY + 18,
    { align: "right" }
  );
  ...
  doc.text("Accountant's Signature", x + width - margin, signatureY, {
    align: "right",
  });
  doc.line(
    x + width - margin - 30,
