...
export async function generateBillsPdf(
  ...
  const logoData =
    logoBase64 ??
    (bills[0]?.school.schoolLogoUrl
      ? await getClientImageData(bills[0].school.schoolLogoUrl)
      : null);
  ...
  for (const [index, bill] of bills.entries()) {
    ...
    doc.text(school.schoolName || "School Name", headerTextX, 20, {
      align: headerAlign,
    });
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(school.schoolAddress || "School Address", headerTextX, 28, {
      align: headerAlign,
    });
    doc.text(`Phone: ${school.schoolPhone || "N/A"}`, headerTextX, 33, {
      align: headerAlign,
    });
    ...
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("CASH BILL", doc.internal.pageSize.getWidth() / 2, 50, {
      align: "center",
    });
    ...
    doc.text(`Bill No: ${invoice.id.slice(-6).toUpperCase()}`, 195, 65, {
      align: "right",
    });
    doc.text(`Date: ${format(new Date(), "yyyy-MM-dd")}`, 195, 72, {
      align: "right",
    });
    doc.text(`Month: ${invoice.month}, ${invoice.year}`, 195, 79, {
      align: "right",
    });
    ...
    const tableData = invoice.lineItems
      ...
      .map((item, i) => [
        ...
        item.feeType
          .replace(/([A-Z])/g, " $1")
          .replace(/^./, (str) => str.toUpperCase()), // Prettify fee type
    ...
    autoTable(doc, {
      ...
      foot: [
        [
          "",
          "Current Month Total",
          `Rs. ${currentMonthTotal.toLocaleString()}`,
        ],
      ],
